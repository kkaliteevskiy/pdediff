...
def get_guidance(xt, y, t, n_samples = 100, sigma_y = sigma_y, use_normaliser = True, gamma = 0.1):
    ...
    if use_normaliser:
        normaliser = 1/(1+ sde.sigma(t)**2/sde.mu(t)**2 * gamma)  # / (sigma_y**2 + sde.sigma(t)**2/sde.mu(t)**2 * gamma)
...
def get_guidance_IS(y, xt, t, n_samples = 1, sigma_y = sigma_y, use_normaliser = True, gamma = 0.1): 
    '''
    ...
    if use_normaliser:
        normaliser = 1/(1+ sde.sigma(t)**2/sde.mu(t)**2 * gamma)# sigma_y # /(sigma_y**2 + sde.sigma(t)**2/sde.mu(t)**2 * gamma)
        grads = grads * normaliser
    return grads, x_hats
...
def get_guidance_IS_2(y, xt, t, n_samples = 1, sigma_y = sigma_y, use_normaliser = True, gamma = 0.1):
    '''
    Importance sampling guidance estimation
    Linearisation is done about the Tweedie mean
    ...
    if use_normaliser:
        normaliser = 1/(sde.sigma(t)**2/sde.mu(t)**2 * gamma + 1)
        grads = grads * normaliser

    return grads , x_hats

def get_guidance_DPS(y, xt, t, sigma_y, gamma = 0.1):
    '''DPS guidance estimation'''
    mu = get_mu_from_xt(xt, t)
    x_hats = mu.unsqueeze(0)
    log_p_est = -(y-A(x_hats)).pow(2).sum(dim=0)/2
    grads = torch.autograd.grad(log_p_est, xt, torch.ones_like(log_p_est))[0]
    normaliser = torch.norm(y - A(x_hats), p=2, dim = 0)
    return grads/normaliser, x_hats

def get_guidance_SDA(y, xt, t, sigma_y, gamma = 0.1):
    '''SDA guidance estimation'''
    mu_t, sigma_t = sde.mu(t), sde.sigma(t)
    mu = get_mu_from_xt(xt, t)
    x_hats = mu.unsqueeze(0)
    log_p_est = -(y-A(x_hats)).pow(2).sum(dim=0)/2
    grads = torch.autograd.grad(log_p_est, xt, torch.ones_like(log_p_est))[0]
    normaliser =  sigma_y**2  + gamma * (sigma_t/mu_t)**2 
    return grads/normaliser, x_hats
...
def get_targert_hist(y, sigma_y, n_bins):
    """Compute the target distribution p(x|y) using histogram method
    Args:
        y (tensor): observation
        sigma_y (float): observation noise
        n_bins (int): number of bins for histogram
    Returns:
        posterior_bar (tensor): counts target distribution p(x|y)
        edges (tensor): bin edges
        Dx (float): bin width
    """ 
